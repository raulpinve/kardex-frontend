...
const PlanCard = ({ name, description, companies, auditors, setDrawerOpen, setPlanSeleccionado }) => {
	const { plan: planActual, estado } = useSelector(state => state?.suscripcion?.suscripcion || {});
	...
	const isCurrent = planActual === planKey && estado === "activo";
	const isUpgrade = nivelPlan > nivelActual;
	const isDowngrade = nivelPlan < nivelActual;

	// Texto del botón según la comparación de niveles
	let botonTexto;
	if (isCurrent) {
		botonTexto = "Actual";
	} else if (isUpgrade) {
  ...
  return (
    <div className="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white rounded-lg border border-gray-100 dark:border-gray-800 shadow xl:p-8 dark:bg-gray-900 dark:text-white relative">
      ...
      <button
            className={`text-white font-medium rounded-lg text-sm px-5 py-2.5 text-center cursor-pointer
            ${isCurrent ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-primary-200"}`}
            onClick={() => {
              if (!isCurrent) {
                  setDrawerOpen(true);
...
const infoPlanes = [
    {
        name: "Básico",
        description: "Ideal para pequeñas empresas que buscan un control simple y ordenado de su kardex.",
        price: "99.900",
    ...
    {
        name: "Estándar",
        description: "Perfecto para organizaciones en crecimiento que necesitan más control y flexibilidad.",
        price: "149.900",
        companies: "5 almacenes",
        auditors: "20 usuarios",
    },
    {
        name: "Premium",
        description: "Pensado para entidades con alta demanda y gestión avanzada de auditorías e inventario.",
        price: "199.000",
        companies: "15 almacenes",
...
const MiPlanPage = () => {
    ...
    const suscripcion = useSelector(state => state?.suscripcion?.suscripcion);
    const planActual = useSelector( state => state.suscripcion.suscripcion.plan);

    const fechaFin = suscripcion?.fechaFin ? new Date(suscripcion.fechaFin) : null;
            ...
            <div className="py-8 px-4 mx-auto max-w-screen-xl">
              {suscripcion?.plan && suscripcion?.fechaFin && (
                ...
                ">
                  ...
                  <span className="capitalize">{suscripcion.estado}</span>
                ...
                <div className="space-y-8 lg:grid lg:grid-cols-3 sm:gap-6 xl:gap-10 lg:space-y-0">
                    {infoPlanes.map((plan) => (
                        <PlanCard 
                            ...
                            {...plan} 
                            isCurrent={NOMBRES_PLANES_PARSEADOS[plan.name] === planActual}
                            setPlanSeleccionado = {setPlanSeleccionado}
                        />
                    ))}
                </div>
                <DrawerPlan drawerOpen={drawerOpen} setDrawerOpen={setDrawerOpen} planSeleccionado= {planSeleccionado} PLANES = {PLANES}/>
            </div>
        </Card>
    );
};
