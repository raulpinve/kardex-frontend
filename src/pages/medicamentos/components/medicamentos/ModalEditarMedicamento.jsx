...
import { editarMedicamento } from "../../services/medicamentosServices";

const ModalEditarMedicamento = (props) => {
    const { cerrarModal, setMedicamentos, medicamentoSeleccionado} = props;
    ...
    const onSubmit = async(values) => {
        ...
        setLoading(true)
        try {
            const result = await editarMedicamento(token, medicamentoSeleccionado.id, values)
            const data = result?.data
            if(data){
                setMedicamentos(prevMedicamentos =>
                    prevMedicamentos.map(medicamento => {
                        return medicamento.id === data.id ? { ...medicamento, ...data } : medicamento
                    })
                );
                cerrarModal()
                setValue("nombre", "");
                setValue("formaFarmaceutica", "");
                setValue("concentracion", "");
    ...
    useEffect(() => {
        if(medicamentoSeleccionado){
            setValue("nombre", medicamentoSeleccionado.nombre);
            setValue("formaFarmaceutica", medicamentoSeleccionado.formaFarmaceutica);
            setValue("concentracion", medicamentoSeleccionado.concentracion);
            setValue("presentacionComercial", medicamentoSeleccionado.presentacionComercial);
            setValue("unidadMedida", medicamentoSeleccionado.unidadMedida);
